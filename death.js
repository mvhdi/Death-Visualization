    ...
    var newSlice = slice.enter()
        ...
        .on('click', d => {
            ...
            if (parNode == "Male") {
                ...
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/MenBlack.txt", "Categories of death by age [Black Men]"); 
                } 

                if (curNode == "Native" ) {
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/MenNative.txt", "Categories of death by age [Native American Men]");  
                }

                if (curNode == "Hispanic") {
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/MenHispanic.txt", "Categories of death by age [Hispanic Men]");  
                }

                if (curNode == "White") {
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/MenWhite.txt", "Categories of death by age [White Men]");  
                }

                if (curNode == "Asian") {
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/MenAsian.txt", "Categories of death by age [Asian/Pacific Islander Men]");  
                }
            ...
            if (parNode == "Female") {
                ...
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/WomenBlack.txt", "Categories of death by age [Black Women]"); 
                } 

                if (curNode == "Native" ) {
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/WomenNative.txt", "Categories of death by age [Native American Women]");  
                }

                if (curNode == "Hispanic") {
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/WomenHispanic.txt", "Categories of death by age [Hispanic Women]");  
                }

                if (curNode == "White") {
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/WomenWhite.txt", "Categories of death by age [White Women]");  
                }

                if (curNode == "Asian") {
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/WomenAsian.txt", "Categories of death by age [Asian/Pacific Islander Women]");  
                }
            ...
            if (parNode == "Age") {
                ...
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/<1.txt", "Categories of death by age [<1 year]"); 
                } 
                if (curNode == "1-4"){
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/1-4.txt", "Categories of death by age [1-4 years]"); 
                } 
                if (curNode == "5-14"){
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/5-14.txt", "Categories of death by age [5-14 years]"); 
                } 
                if (curNode == "15-24"){
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/15-24.txt", "Categories of death by age [15-24 years]"); 
                } 
                if (curNode == "25-34"){
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/25-34.txt", "Categories of death by age [25-34 years]"); 
                } 
                if (curNode == "35-44"){
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/35-44.txt", "Categories of death by age [35-44 years]"); 
                } 
                if (curNode == "45-54"){
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/45-54.txt", "Categories of death by age [45-54 years]"); 
                } 
                if (curNode == "55-64"){
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/55-64.txt", "Categories of death by age [55-64 years]"); 
                } 
                if (curNode == "65-74"){
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/65-74.txt", "Categories of death by age [65-74 years]"); 
                } 
                if (curNode == "75-84"){
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/75-84.txt", "Categories of death by age [75-84 years]"); 
                } 
                if (curNode == "85+"){
                d3.selectAll("#the_SVG_ID").remove();
                d3.selectAll("#daTitle").remove();
                makeBar("data/85+.txt", "Categories of death by age [85+ years]"); 
                } 
            }         
...
d3.csv(dataFile, function(error, data) {
    ...
    .on("mouseover", function(d) {
        ...
        var texxt = d.y1 - d.y0;
        return tooltip.style("visibility", "visible").text('[ ' + d.name +' ]'+ ' [ ' + texxt +']' );
    ...
    .data(data[0].genes)
        ...
        return tooltip.style("visibility", "visible").text('[ category:' + d.name +']' );
...
//     10: ["#9e0142","#d53e4f","#f46d43","#fdae61","#fee08b","#e6f598","#abdda4","#66c2a5","#3288bd","#5e4fa2"],
//     11: ["#9e0142","#d53e4f","#f46d43","#fdae61","#fee08b","#ffffbf","#e6f598","#abdda4","#66c2a5","#3288bd","#5e4fa2"],
//     15: ["#1E77B4","#1E774B","#AEC6E8","#FF7F0D","#FEBB78","#2B9F2C","#98DE8A","#D52628","#9367BD","#C4B0D5","#BCBC21","#DBDB8D", "#17BECE", "#9EDAE5", "#C7C7C7"]
// }};
...
// var x = d3.scaleBand()
//     .rangeRound([0, width * (15 / 18)], .08).padding(.1);
...
//         var texxt =  Number((d.y1 - d.y0).toFixed(4));
...
// makeBar("data/catOfDeath.txt", "categories of death by age");
